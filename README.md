...
```console
$ v=v0.9.0; t=aarch64-apple-darwin     # or x86_64-apple-darwin, x86_64-unknown-linux-musl, aarch64-unknown-linux-musl
...
there are no dependencies. In CI, `uses: i2y/rulec@v0.9.0` does the download and the check
([In CI](#in-ci)).
...
- uses: i2y/rulec@v0.9.0                     # the release binary, verified against its checksum
...
28 rules taken from real published terms are checked, generated and run on every commit, and all 72 diagnostics are implemented. What is built:
