...
```console
$ v=v0.15.0; t=aarch64-apple-darwin     # or x86_64-apple-darwin, x86_64-unknown-linux-musl, aarch64-unknown-linux-musl
...
there are no dependencies. In CI, `uses: i2y/rulec@v0.15.0` does the download and the check
([In CI](#in-ci)).
...
- uses: i2y/rulec@v0.15.0                     # the release binary, verified against its checksum
...
tests/corpus/     44 rules, and the copies of the documents they cite
...
44 rules — 36 taken from real published terms, 8 written to reach the rest of the language — are checked, generated and run on every commit, and all 77 diagnostics are implemented.
