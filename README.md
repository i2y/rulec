...
**一件の取引を、決まった数の平たい事実から一度で判定する規則**です。返ってくる答えは四種類 — **金額・可否・区分・順序**。
...
決め手は**判定の形**であって、扱う値が何かではありません。セルが自分の列だけを見るテストだから、行が「箱」になり、抜けと重なりを厳密に決められる — そこが境界です。だから「送料のための道具」でも「EC のための道具」でもありません。

**ただし、数を返すなら単位が要ります。** 数値の型は**数量（g・cm）・金額・率**の三つで、**単位のない裸の数**（件数、日数、点数）を返す型がありません。「何日後に出荷か」は区分（列挙）にするか、いまは書けません。
...
| **書く人と決める人が違う** | 金額も、丸めの向きも、区分の切れ目も業務の判断。だから承認する人に見せる描画があり、検査は「決められないこと」を具体例つきの質問に変える |

### 言語としての狙い

**一つのファイルに三役を兼ねさせること**です。同じ `.rule` が、承認する人が読む**仕様**であり、検査器が証明する**対象**であり、生成コードの**原本**。この三つが別々のファイルに分かれた瞬間、どれかが必ず腐ります。そして腐るのは、たいてい仕様のほうです。
...
**書けません — 定価の比で割り付ける形。** 「明細の定価の比で按分する」には `明細 ÷ 合計` が要りますが、**割り算は定数でしか書けません**。`注文金額 / 100円` は書けても、`明細 / 合計` は書けない。比率を呼び出し側で計算して率として渡す迂回も、いまは率の刻みが 1% 単位までしか書けないので実用になりません。

### 向いていないもの

- **ワークフロー** — 複数の段階、状態を持つもの
- **集合そのものの判定** — 「明細のどれかが冷蔵品」「カート全体で 3 点以上」。呼び出す手前で平たくしてから渡します
