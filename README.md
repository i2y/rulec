...
`.rule` ファイルは**上から順に決まった形**をしています。ヘッダ、取込、型と群、入力、出力、導出と定義と表、結果、例。前方参照はできないので、上から読めば依存が読めます。

### 型

閉じた列挙を宣言します。**開いた列挙はありません** — 値が増えたら、それを見ていない表が完全性検査で割れるのが狙いです。

```
型 会員区分(member_kind) = 一般(basic) 既定扱い | ゴールド(gold) 既定扱い | プラチナ(platinum)
```

括弧の中は **ASCII 別名**で、生成される Python と Go の公開名になります（漢字は Go の公開識別子になれないため）。必須なのは規則名・入力・出力とその型だけで、内部の名前は日本語のままで構いません。

`既定扱い` は「この値に専用の行は要らない、`-` に吸われるのが正しい」という宣言です。付けないと「どの行にも現れません」と警告されます。

**群**は列挙の名前つき部分集合で、セルの中で値と同じように使えます。
...
- **数量と金額の入力には `範囲` が必須**です。オーバーフローの証明、完全性検査の全体集合、生成コードの入口ガードの三つを兼ねます
- **数値の出力には `丸め` が必須**です。端数がどう決まるかを宣言しないと、生成コードが黙って決めてしまうので
- 範囲の入口検査としてしか効かない入力は `契約のみ` を付けて、未使用の警告を黙らせます

丸めは四種で、**負の向きまで固定**されています。切り上げは 0 から遠ざけ（−4.2 → −5）、切り捨ては 0 へ寄せ（−4.8 → −4）、四捨五入は半分ちょうどを 0 から遠ざけ、銀行家丸めは偶数へ。Python の `//` は −∞ 方向、Go の整数除算は 0 方向で食い違うので、言語の素の除算には任せません。
...
「クーポン適用後の金額が 3,980 円以上なら」のような、値引き後で判定する規約が実在します。これを列に書けないと、いちばん間違えやすい引き算が呼び出し側の裸の一行になってしまいます。範囲の宣言は必須で、**入力範囲から計算した到達区間を含んでいないとエラー**です。
...
**M0 から M3 まで、四段すべて完成しています。** テスト 89 本、コーパス 7 本、診断は台帳の全項目。
