...
**いつ出るか。** `policy first` の表で、部分交差していて出力が違う行の対があるとき。階段状の隠れと、答えが同じ隠れは件数の注記に畳まれ、ここに一覧されるのは要確認の対だけです（§4）。
...
**いつ出るか。** 入力・導出・グループ・列挙の値が、どの表のどのセルにも現れないとき。書き忘れの徴候であることも、正当な契約であることもあります。取込した型の値は対象外です。
