...
![The agent writes the table, rulec proves it and returns what to fix, and only what cannot be decided goes to a person. Out come Python, TypeScript, JavaScript, Rust, Ruby, Go, Swift, SQL and Wasm functions generated from the proved table, and the impact known before you deploy](images/flow.svg?v=28bb0853#only-dark)

![The agent writes the table, rulec proves it and returns what to fix, and only what cannot be decided goes to a person. Out come Python, TypeScript, JavaScript, Rust, Ruby, Go, Swift, SQL and Wasm functions generated from the proved table, and the impact known before you deploy](images/flow-light.svg?v=28bb0853#only-light)
...
<div class="rc-overview" markdown>
![What one table produces is a column of the next: band_of turns the distance and whether the flight is intra-EU into a band, and amount turns that band into the compensation. Not every table is in the chain — reduction reads the rule's inputs directly — and result puts the two together](images/stack.svg?v=28bb0853#only-dark)
...
by types and declarations, nine languages out with zero dependencies, comparison against the
...
<div class="rc-overview" markdown>
![One computation decides three defects: a gap (E101) is a stretch no row covers, an overlap (E105) is a stretch two rows both cover, and an unreachable row (E102) is one whose whole stretch the earlier rows take first. Same table in all three; one thing changed](images/checks.svg?v=28bb0853#only-dark)
![One computation decides three defects: a gap (E101) is a stretch no row covers, an overlap (E105) is a stretch two rows both cover, and an unreachable row (E102) is one whose whole stretch the earlier rows take first. Same table in all three; one thing changed](images/checks-light.svg?v=28bb0853#only-light)
</div>
