...
- **`rulec test`** runs the generated Python, TypeScript, Rust, Ruby and Go
  and compares them with the reference evaluator. It needs `python3`, `node`,
  `rustc`, `ruby` and `go` on the path; without one it says which side it skipped and
  does not fail.
