...
| **8. The repository's own tests** | 91 deliberately broken rules each produce the diagnostic they should; 46 rules are checked, generated and run on every commit | `cargo test` |
...
- **91 deliberately broken rules**, each producing the diagnostic it should — and the
  expected codes are pinned, so a mutant that starts reporting something else fails.
- **46 rules** — 36 transcribed from real published terms, 9 written to reach the corners of
...
- **Every diagnostic is in the ledger**: 81 codes, each with a smallest reproduction that is
