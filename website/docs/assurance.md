...
| **8. The repository's own tests** | 83 deliberately broken rules each produce the diagnostic they should; 35 rules are checked, generated and run on every commit | `cargo test` |
...
- **83 deliberately broken rules**, each producing the diagnostic it should — and the
  expected codes are pinned, so a mutant that starts reporting something else fails.
- **35 rules** — 28 transcribed from real published terms, 7 written to reach the corners of
