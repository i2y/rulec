...
出るのは普通の Python・TypeScript・JavaScript・Rust・Ruby・PHP・Swift のモジュールと、Java のクラスと、普通の Go パッケージと、SQL の問い合わせと関数と、Wasm の一つのモジュールです。ランタイムも設定も要らず、標準ライブラリの外に依存もありません。最後の一つは、言っているだけでなく**確かめています**。`rulec test` は Go 側を `GOPROXY=off` で走らせるので、外から何かを取ってこようとすれば落ちます。例外は二つです。NumPy では、出るのはコードではなく、規則そのものと、それを読む固定の評価器で、こちらは numpy に依存します。列ごとまとめて判定したいホストが、もう持っているはずのものです。もう一つは Connect のサービスで、`connectrpc` に依存します（下の「ほかのチームが呼ぶサービスにする」）。
...
一つだけ決めていることがあります。**一致検査に乗らない言語は入れません。** 参照評価器とバイト単位で突き合わせられない生成物は、「証明済み」という看板の外側にあることになるからです。三つめの TypeScript を足すのに掛かった実コストは生成器に約 700 行で、五つめの Ruby も六つめの Swift も同じくらいでした。違ったのは、生成器の外で手を入れたところです。Ruby のときは、コードとテストに言語を並べた一覧が七つあり、図と文書まで含めて二十数ファイルを手で直しました。Swift のときは、その七つを対象言語の一覧一か所にまとめたあとだったので、コードの中で足したのはその一覧の一項目だけでした。
...
この関数が出るのは、12 の対象のうち五つです。Python・TypeScript・JavaScript・Ruby・PHP では、読み込んだ JSON を連想配列（Python なら dict、Ruby なら Hash）のまま使うことが多いので、関数の引数も連想配列にしてあります。呼び出し側の型を知らなくても受け取れます。Go・Swift・Java・Rust では、呼び出し側はオブジェクトを構造体やクラスで持っているので、それを受け取る関数には、その型の名前を書かなければなりません。その型は呼び出し側のものなので、rulec は、同じ型を自分で生成することも、protoc などが生成した型に合わせてコードを書くこともしません。この四つでは、入力を取り出して規則の関数を呼ぶところを、呼び出し側が書きます。SQL と NumPy は列を、Wasm のモジュールは規則自身の入力を並べた JSON を受け取るので、射影する相手のオブジェクトがありません。パスを契約に照らす検査は、`rulec check` の中で起きるので、12 の対象のどれにも効きます。
...
モデル検査器と、表を証明した検査器は、コードを一行も共有していません。走らせる値打ちはそこにあります。一致すれば、無関係な二つの道具が同じことを言ったことになります。食い違えば、どちらかが誤っていて、それを示す入力がそのまま出てきます。コーパス 50 本では、ハーネス 118 本が 185 秒で通ります。
...
**MCP Apps** を表示できるホストには、同じサーバがもう一つ渡します。承認者向けのページを、そのツールの view として渡すのです。`gen` がサーバの隣に書く `shipping_fee_page.html`（`rulec doc --format html` が出すページそのもの）で、ホストはそれを、**いま訊かれた件が入った状態で**開きます。フィールドは埋まっていて、答えが出ていて、決めた行に色が付いています。チャットを読む人に見えるのが、数字だけでなく「どの表のどの行がそう言ったか」になります。ホストが明るい配色か暗い配色かを伝えてくれば、ページもそれに合わせます。
...
`connectrpc` は、NumPy の評価器が使う numpy を除けば、`gen` の出すものの中で唯一よその依存で、それもサービスの一枚に閉じています。呼ばれるモジュールのほうは相変わらず何も import しません。`rulec test` は全ベクタを、二つのアプリそれぞれに POST と GET で、合わせて四回通して参照評価器と突き合わせます。
...
ベクタ 70 件
  ...
  丸めの同着カバー              0 / 0     満たす
  畳み込みの遷移カバー          0 / 0     満たす
  ステートマシンの遷移カバー    0 / 0     満たす
```

`coverage` は**テストの側に対する完全性検査**です。七種類の条件は**規則のほうから先に**導きます。ベクタを見てから条件を決めるのでは、何も確かめたことになりません。欠けていればどの行・どの境界・どの隠れの対・どの計算値・どの丸めの同着・どの畳み込みやステートマシンの遷移かを名指しして 1 で落ちます。

計算値とは、リテラルではなく名前を返す行と、`define` や `result` の行で計算する出力のことです。入力を一つだけ変えた二つのベクタで、実装が返す値として二通りに試されていなければなりません。そうでないと、そこで定数を返す実装でも、どのベクタとも一致してしまいます。ただし、どんな入力でも丸めると同じ額になる出力のように、一つの値にしかならないものには義務が立ちません。支払額をそのまま返金する行が、支払額 0 円でしか試されていなかったのが、この条件を足したきっかけです。

丸めの同着は、刻みのちょうど半分ずれた値のことです。`half_up` と `half_down` の答えが分かれるのはこの一点だけなので、ここに載る入力が無いと、二つを入れ替えても期待値が変わりません。丸めを宣言した出力には、どれも同着の義務が立ちます。立たないのは、どの入力も届かないと示せるときだけです。示し方は、行ごとにその行が当たる範囲の中で計算するか、入力が少なければ全部を評価するかです。18.3% × 標準報酬月額は必ず偶数の円なので、折半額に端数が出ない厚生年金保険料は 0 / 0 です。見つけにくいだけの同着は義務として残り、欠けとして出ます。
