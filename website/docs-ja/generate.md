...
**MCP Apps** を表示できるホストには、同じサーバがもう一つ渡します。承認者向けのページを、そのツールの view として渡すのです。`gen` がサーバの隣に書く `shipping_fee_page.html`（`rulec doc --format html` が出すページそのもの）で、ホストはそれを、**いま訊かれた件が入った状態で**開きます。欄は埋まっていて、答えが出ていて、決めた行に色が付いています。チャットを読む人に見えるのが、数字だけでなく「どの表のどの行がそう言ったか」になります。
...
生成する Rust は依存が無く、外に触れるのは標準入出力だけなので、`wasm32-wasip1` 向けにそのまま組めます。`wasmtime` が PATH にあり、対象の標準ライブラリが入っていれば（`rustup target add wasm32-wasip1`）、`rulec test` は Rust の runner をもう一度 WASI のモジュールとして走らせ、同じ期待値と突き合わせます。
...
wasmtime が無ければこの段は飛ばして、そう言います。言語が欠けたのとは別に数えます。

Shopify Functions、Extism のプラグイン、Cloudflare Workers のような置き場が規則に与えるのは、まさにこの形です — 小さなモジュール一つ、ネットワーク無し、命令数の上限。規則のモジュールは置き場の crate にそのまま入り、置き場ごとの部分 — カートを読んで規則の入力に平らに潰す、出力を割引の操作に直す — は、その隣の二十行の境界に残ります。どこまでが境界で、そういう関数に決められるもの（割引）と決められないもの（配送料）は何かは、[一覧に無い言語へ生成する](backends.md#a-wasm-host-shopify-functions) にあります。
