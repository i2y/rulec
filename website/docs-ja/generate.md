...
計算値とは、リテラルではなく名前を返す行と、`define` や `result` の行で計算する出力のことです。入力を一つだけ変えた二つのベクタで、実装が返す値として二通りに試されていなければなりません。そうでないと、そこで定数を返す実装でも、どのベクタとも一致してしまいます。支払額をそのまま返金する行が、支払額 0 円でしか試されていなかったのが、この条件を足したきっかけです。

丸めの同着は、刻みのちょうど半分ずれた値のことです。`half_up` と `half_down` の答えが分かれるのはこの一点だけなので、ここに載る入力が無いと、二つを入れ替えても期待値が変わりません。丸めを宣言した出力には、どれも同着の義務が立ちます。立たないのは、規則の計算からどの入力も届かないと示せるときだけです。18.3% × 標準報酬月額は必ず偶数の円なので、折半額に端数が出ない厚生年金保険料は 0 / 0 です。見つけにくいだけの同着は義務として残り、欠けとして出ます。
