...
この関数が出るのは、12 の対象のうち五つです。Python・TypeScript・JavaScript・Ruby・PHP では、読み込んだ JSON を連想配列（Python なら dict、Ruby なら Hash）のまま使うことが多いので、関数の引数も連想配列にしてあります。呼び出し側の型を知らなくても受け取れます。Go・Swift・Java・Rust では、呼び出し側はオブジェクトを型（構造体やクラス）で持っていて、その型を名指すには、型を生成するか、呼び出し側の型を追いかけるかしかありません。この道具はどちらもしません。SQL と NumPy は列を、Wasm のモジュールは規則自身の入力を並べた JSON を受け取るので、射影する相手のオブジェクトがありません。パスを契約に照らす検査は、`rulec check` の中で起きるので、12 の対象のどれにも効きます。
