...
!!! note "書くのは、たいていエージェントです"
    ...
    そして読むときも一人ではありません。承認用の資料は `rulec doc` が出します — 表の字面には無いのに検査器が知っている事実（このグループとその「以外」で 47 値を覆いきっていること、どの行がどの行を隠しているか、どの丸めが根拠なしの仮置きか）が添えられます。変更なら「何件がいくら動くか」も入れる前に出ます。どちらもエージェントに頼めるもので、分からない行はその場で訊けます。エージェント側の手順は[エージェント向け](agents.md)にあります。
