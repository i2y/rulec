...
`source` は文書の宣言です。法令なら、政府の法令データベース（e-Gov 法令検索）の法令 ID と、いつの時点の条文かを `asof` で書きます。隣に置いたファイルなら `file "料金表.pdf" sha256:…` です。表・節・行・導出・定義は、行末に `@出典 箇所` と書いて引用します。箇所は `第20条`、`第20条の2`、`第20条第2項第3号`、`別表第一` のように、原文の呼び方のままです。

引用した箇所は、`rulec source fetch` が e-Gov から取ってきて、規則の隣の `sources/` に写しを置きます。`rulec source pin` が、その写しのハッシュを `source` の下の行に書き込みます。以後 `rulec check` は毎回、写しがあること、写しのハッシュが書いてあるとおりであることを確かめます。写しを取り直して条文が変わっていれば、その箇所を引用している表・節・行を名指しして止まります（E038）。読み直すのはそこだけで済みます。check 自体は通信しません。後の改正で条文が変わるかどうかは、`rulec source outdated` が e-Gov に問い合わせます。CI の定期実行に置く想定です。
