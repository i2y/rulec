...
`rulec source outdated` は、元の文書が動いたかどうかを問い合わせるコマンドです。法令なら `asof` より後に施行される改正で条文の本文が変わるかを e-Gov 法令検索に訊き（XML の属性だけの違いは改正と数えません）、ファイルなら `url "…"` の先を見て、**引いている表が変わったのか、この規則が写していないところが変わっただけなのか**を言います。写しを取り直さない限り check は改正を知りようがないので、CI の定期実行に置く想定です。
