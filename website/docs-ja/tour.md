...
| `source` | 規則が写した文書。e-Gov の法令（`law "<法令ID>" asof <日付>`）か、隣に置いたファイル（`file "<ファイル>" sha256:…`）。表・節・行・導出・定義の行末に `@出典 第20条` と書いて引用する |
...
`source` は文書の宣言です。e-Gov の法令なら法令 ID と、いつの時点の条文かを `asof` で書きます。隣に置いたファイルなら `file "料金表.pdf" sha256:…` です。表・節・行・導出・定義は、行末に `@出典 箇所` と書いて引用します。箇所は `第20条`、`第20条の2`、`第20条第2項第3号`、`別表第一` のように、原文の呼び方のままです。
