...
料金表には、本則と、それに優先する特例がつきものです。「契約金額が 10 万円を超える契約書は、令和 9 年 3 月 31 日までは軽減税率」のような決まりです。一つの表に `軽減期間` の列を足して書くこともできますが、原文が二つ（印紙税法の別表第一と、租税特別措置法の第 91 条）なら、表も二つに分けたほうが原文と見比べられます。
...
`overrides 本則` は「この表の行は、表 本則 の行に優先する」という宣言です（抜粋なので行は一部です）。行の頭の `r1` はラベルで、`overrides 本則:r4` のように行を指したいときに使います。
...
条件が列に並ばない一行の決まり — ただし書がそうです — は、表にせず `clause` で書きます。
...
節は**一行の表**として扱われます。検査も生成も記録も表と同じ機構で、記録には `{"table":"無料","row":1}` と出ます。表と混ぜて `overrides` でつなげます。

## 出典を書き、写しに縛る — source と @
...
| **法令** | `source 法 = law "<法令ID>" asof <日付>`。法令 ID は e-Gov 法令検索（政府の法令データベース）のもの、`asof` はいつの時点の条文か | `@法 第91条` のように、条や別表を必ず書く。書き方は `第20条`、`第20条の2`、`第20条第2項第3号`、`別表第一` と、原文の呼び方のまま | `rulec source fetch` が e-Gov 法令検索から条文を取り、規則の隣の `sources/` に箇所ごとの写しを置く。`rulec source pin` がそのハッシュを `source` の下の行に書く |
...
法令の出典についてだけ、もう一つコマンドがあります。`asof` より後に施行される改正で引用した条文が変わるかどうかを、`rulec source outdated` が e-Gov 法令検索に問い合わせます。写しを取り直さない限り check は改正を知りようがないので、CI の定期実行に置く想定です。隣に置いたファイルは、このコマンドの対象ではありません。ファイルが差し替わったかどうかは、check がハッシュの違いでその場で見つけます。
...
見出しに、準用する規則のファイルと、そのファイルのハッシュを書きます。下の行が読み替えです。元の規則の入力を一つ残らず、この規則の何にあてるかを書きます（入力、導出、定義、前の表の出力、リテラルのどれか）。列挙どうしは `with` で値を対応づけます。同じ綴りの値は書かなくて構いません。`except` は、元の規則の定義のうち準用しないもの（「第 20 条（第 2 項を除く。）」の形）です。元の規則の出力はこの規則の値になり、`->` で名前を付け替えられます。

`rulec check` は、元の規則を自分の場所で丸ごと検査してから、その表と節を `退職手当:支給表` のような名前でこの規則の中に展開し、一つの規則として検査します。だから完全性も重なりも、準用したあとの形で証明されます。加えて三つを確かめます。読み替えに漏れが無いこと（E041）と型が合うこと（E042）、そして**この規則が渡す値が、元の規則の範囲に収まること**（E043）。在職期間の範囲を 0 から書いてしまえば「在職期間 = 0 は、退職手当.rule の 勤続年数 の range >=1 <=40 の外です」と止まります。元の規則の完全性はその範囲の上でしか証明されていないからで、範囲を狭めるか、はみ出す分をこの規則の節で定めるかは業務の判断です。

元の規則が改正されると、ハッシュが合わなくなって E040 で止まります。`rulec diff` でこの規則の答えが何件いくら動くかを見て、それでよければ `rulec source pin` でハッシュを書き直します。元の規則の表のうち、この規則の範囲では当たらない行は、エラーにはせず、承認用の資料に「この準用では当たらない行」として挙がります。表の全行が当たらないときだけ、W118 で知らせます。
