...
法令の出典についてだけ、もう一つコマンドがあります。`asof` より後に施行される改正で引用した条文が変わるかどうかを、`rulec source outdated` が e-Gov 法令検索に問い合わせます。写しを取り直さない限り check は改正を知りようがないので、CI の定期実行に置く想定です。隣に置いたファイルにはこの問いはありません。差し替われば、ハッシュが変わったことを check がその場で見ます。
