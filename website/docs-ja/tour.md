...
以後 `rulec check` は毎回、写しがあること、写しのハッシュが書いてあるとおりであることを確かめます。写しが変わっていれば — ファイルを差し替えた、条文を取り直したら改正されていた — その箇所を引用している表・節・行を名指しして止まります（E038）。読み直すのはそこだけで済みます。check 自体は通信しません。後の改正で条文が変わるかどうかは、`rulec source outdated` が e-Gov 法令検索に問い合わせます。CI の定期実行に置く想定です。
