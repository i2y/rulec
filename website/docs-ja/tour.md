...
!!! note "書くのは、たいていエージェントです"
    ...
    そして読むときも一人ではありません。承認用の資料は `rulec doc` が出します — 表を読んだだけでは分からないことが、そこに添えられます。表にひとまとめで書いてある一語が、具体的には何を指しているのか。どの行が、手前の行に隠されているのか。どの丸めが、根拠のない仮置きなのか。変更なら「何件がいくら動くか」も、入れる前に出ます。どちらもエージェントに頼めるもので、分からない行はその場で訊けます。エージェント側の手順は[エージェント向け](agents.md)にあります。
