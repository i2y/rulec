...
一行ぶんは「ここまでの配分」から「直前までの配分」を引いた差です。この形なら端数は最後の行に寄り、**配った合計は総額にぴったり一致します**。`proofs/` にその定理があります。求めるのは、範囲を宣言した名前が三つ、配る額と累計が負にならないこと、全体が正であること、そして上の `constraint` です。足りなければ E117 で止まります。
