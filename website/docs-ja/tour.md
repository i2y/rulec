...
| `overrides` | その表の全行が優先する、上で宣言した表か行（`表:行ラベル`）。`policy` の次に書く |
| `clause` | 一行の定義を文で書く。`when <列> <セル> and …`（無条件なら `when always`）、`then <値>`、任意の `overrides` |
| `source` | 規則が写した文書。e-Gov の法令（`law "<法令ID>" asof <日付>`、引いた断片ごとの固定行つき）か、隣のファイル（`file "<ファイル>" sha256:…`）。表・節・行・導出・定義は `@出典 断片` で引く |
| `apply` | ほかの規則を、入力を全部束縛して適用する（準用）。`<呼び先の入力> = <値>`（列挙は `with` で対応）、`except` で外す定義、`<呼び先の出力> -> <名前>`。見出しに呼び先ファイルのハッシュを固定する |
| `result` | 出力の組み立て |
| `examples` | 実行される仕様 |
