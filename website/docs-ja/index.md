...
箱の中の **入 / 出** がそのアクターの受け取るものと渡すもの、矢印の文字が**そこを流れるもの**（動詞ではなく、実際に受け渡されるもの）です。矢印の上の `rulec check` `rulec gen` は、それを起こすコマンド — rulec は自分からは動かず、エージェントが呼びます。
...
① → ② → ④ が**ループ**です。エージェントは診断のとおりに直してまた ① へ戻り、緑になるまで回ります。人が呼ばれるのは ③ のときだけ。

そして大事なのが **③ の矢印**です。ふつうの道具は「分からなかった」を黙って通すか、それらしい既定値で埋めます。rulec は**そこで止まり、具体的な入力を一つ添えて人に渡します** — 「山梨県あての S60 の運賃はいくらですか」。人が答えられる形になっているので、往復が一回で済みます。
