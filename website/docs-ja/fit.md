...
- **並びを足し合わせること** — 「明細の合計が 1 万円を超える」。合計も平均も、要素をまたいで値を持ち越すことになるので書けません。呼び出す手前で出して、値として渡します。**数えることなら書けます**（`count`。「冷蔵品が 3 件以上」は表の行になります）し、並びから**一件を選ぶ**ことも書けます（`fold`。「どれかが冷蔵品なら断る」「いちばん高い行を採る」）
