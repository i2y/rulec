...
`uses: i2y/rulec@v0.2.0` の一行で、そのリリースのバイナリが検査済みで runner の `PATH` に入ります。action を指す ref がそのままリリースなので、二つがずれることはありません。`SHA256SUMS` を信用するのではなくアーカイブそのものを固定したいなら、`with: { sha256: … }` を足します。
