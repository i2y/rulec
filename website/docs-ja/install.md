...
- **`rulec test`** — 生成した Python・TypeScript・JavaScript・Rust・Ruby・Go・Swift・SQL・Wasm を実際に走らせて、参照評価器（rulec の中にある「正解」の実装）と突き合わせます。`python3`・`node`・`rustc`・`ruby`・`go`・`swiftc` が要ります（SQL は同じ `python3` の中の `sqlite3` で走ります）。無ければ「どれを飛ばしたか」を言って、落ちはしません。
