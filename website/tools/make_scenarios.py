...
FIGS["existing"] = dict(
    ja=dict(
        ...
        nodes=[
            ...
            ("actor", ("rulec", ["写しと照合する", "抜けと重なり", "を証明する", "12 言語に生成"]), 150),
            ("sheet", ("生成コード", ["12 言語、依存ゼロ", "ヘッダに出典と", "時点が刻まれる"]), 150),
        ],
...
FIGS["internal"] = dict(
    ja=dict(
        alt="社内の規程や自社サービスの規約、Excel、動いているコードといった手元のものを、エージェントが表（.rule）に写す。Excel からは rulec import で下書きを起こし、文書は @ で表ごとに引用し、写しのハッシュに固定する。rulec が抜けと重なりを証明し、旧実装や過去の記録と突き合わせて、食い違いをどの行で何件いくらかで返す。承認する人は rulec doc の資料で文書と表を見比べる。通った表から12 言語に生成する。",
        nodes=[
            ...
            ("sheet", ("生成コード", ["12 言語、依存ゼロ", "旧実装と同じ答え"]), 150),
        ],
...
FIGS["implementing"] = dict(
    ja=dict(
        alt="検査を通った表（.rule）から rulec gen が12 言語のコードを出し、rulec test が参照評価器と突き合わせる。実装する人は rulec api で呼び方を読み、関数・SQL・Wasm・MCP サーバのどれかを自分のアプリやバッチやエージェントに組み込む。生成物は編集せず、表が変われば CI の gen --check が止める。",
        nodes=[
            ("sheet", ("表", ["検査を通った規則"], ".rule"), 118),
            ("actor", ("rulec", ["12 言語に生成", "参照評価器と", "突き合わせる", "呼び方の一覧"]), 150),
    ...
    en=dict(
        ...
        nodes=[
            ...
            ("actor", ("rulec", ["emits twelve languages", "holds each to the", "reference evaluator", "lists how to call it"]), 160),
