...
FIGS["existing"] = dict(
    ja=dict(
        alt="法令や公開された規約を、エージェントが表（.rule）に写し、条文を @ で引用する。rulec が写しと照合し、抜けと重なりを証明して、九言語に生成する。承認する人は rulec doc の資料で条文と表を見比べる。改正が出れば rulec source outdated が言い、表を読み直す。",
        nodes=[
...
FIGS["designing"] = dict(
    ja=dict(
        ...
        nodes=[
            ...
            ("sheet", ("資料", ["承認者向けの資料", "お客向けの案内", "改定の影響"]), 150),
        ],
...
FIGS["implementing"] = dict(
    ja=dict(
        ...
        nodes=[
            ...
            ("actor", ("rulec", ["九言語に生成", "参照評価器と", "突き合わせる", "呼び方の目録"]), 150),
