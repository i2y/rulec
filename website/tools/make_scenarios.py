...
FIGS["existing"] = dict(
    ja=dict(
        alt="法令や公開された規約を、エージェントが表（.rule）に写し、条文を @ で引用する。rulec が写しと照合し、抜けと重なりを証明して、十二言語に生成する。承認する人は rulec doc の資料で条文と表を見比べる。改正が出れば rulec source outdated が知らせ、表を読み直す。",
        nodes=[
            ("sheet", ("法令・規約", ["e-Gov の条文", "規約の PDF", "料金表"]), 118),
            ("actor", ("エージェント", ["条文を読む", "表に写す", "出典を @ で引く"]), 150),
            ("sheet", ("表", ["@法 第91条", "写しに固定"], ".rule"), 120),
            ("actor", ("rulec", ["写しと照合する", "抜けと重なり", "を証明する", "十二言語に生成"]), 150),
            ("sheet", ("生成コード", ["十二言語、依存ゼロ", "ヘッダに出典と", "時点が刻まれる"]), 150),
        ],
...
FIGS["implementing"] = dict(
    ja=dict(
        alt="検査を通った表（.rule）から rulec gen が十二言語のコードを出し、rulec test が参照評価器と突き合わせる。実装する人は rulec api で呼び方を読み、関数・SQL・Wasm・MCP サーバのどれかを自分のアプリやバッチやエージェントに組み込む。生成物は編集せず、表が変われば CI の gen --check が止める。",
        nodes=[
            ("sheet", ("表", ["検査を通った規則"], ".rule"), 118),
            ("actor", ("rulec", ["十二言語に生成", "参照評価器と", "突き合わせる", "呼び方の一覧"]), 150),
