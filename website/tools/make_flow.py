...
JA = dict(
    alt="エージェントが資料を読んで表（.rule）を書き、rulec check にかける。"
        ...
        "人は金額と丸めの向きを答える。表が通ると、証明済みの表から rulec gen が Python・TypeScript・JavaScript・Rust・Ruby・Go・Swift・SQL・Wasm を出す。",
    ...
    answer=("答え", ["金額", "丸めの向き"]),
    code=("生成コード", ["Python・TypeScript・JavaScript", "Rust・Ruby・Go・Swift", "SQL・Wasm", "依存ゼロ・どれも同じ答え"]),
...
EN = dict(
    alt="The agent reads the sources and writes the table (.rule), then runs rulec check. "
        ...
        "Python, TypeScript, JavaScript, Rust, Ruby, Go, Swift, SQL and Wasm from the proved table.",
    ...
    code=("Generated code", ["Python, TypeScript, JavaScript,", "Rust, Ruby, Go, Swift, SQL, Wasm", "from a proved table, no runtime,", "the same answer from each"]),
    check="rulec check", gen="rulec gen",
