...
agent and rulec, amber for the detour through a person.
    ...
    sources -> Agent -> table -> rulec -> Python, TypeScript, Rust, Ruby, Go, Swift
                 ^                |
...
LANGS = [("Python", "python"), ("TypeScript", "typescript"), ("JavaScript", "javascript"),
         ("Rust", "rust"), ("Ruby", "ruby"), ("Go", "go"), ("Swift", "swift")]
...
JA = dict(
    alt="エージェントが資料を読んで表（.rule）を書き、rulec check にかける。"
        ...
        "人は金額と丸めの向きを答える。表が通ると rulec gen が証明済みの Python・TypeScript・JavaScript・Rust・Ruby・Go・Swift を出す。",
    ...
    answer=("答え", ["金額", "丸めの向き"]),
    code=("生成コード", ["Python・TypeScript・JavaScript", "Rust・Ruby・Go・Swift", "証明済み・依存ゼロ", "どれも同じ答え"]),
...
EN = dict(
    alt="The agent reads the sources and writes the table (.rule), then runs rulec check. "
        ...
        "proved Python, TypeScript, JavaScript, Rust, Ruby, Go and Swift.",
    ...
    code=("Generated code", ["Python, TypeScript, JavaScript,", "Rust, Ruby, Go and Swift", "proved, no dependencies,", "the same answer from each"]),
    check="rulec check", gen="rulec gen",
