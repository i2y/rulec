...
EXAMPLES = [
    ...
    (
        "注文の状態.rule",
        "続いていく注文の、一回の出来事（machine）",
        "通販の注文が、入金・出荷・配達・取消依頼の出来事でどの状態に移り、いくら返金するかを決めます。状態は呼び出す側が持ちます（データベースの注文の行など）。規則は今の状態と一つの出来事を受け取って次の状態を返す純関数のままで、`machine` の節が、その関数を何回続けて呼んでも崩れないことを言います。",
        [
