...
EXAMPLES = [
    ...
    (
        ...
        [
            "**前置きの集合は有限に切れます。** 文字列の全体は無限でも、その列のセルが名指す前置きから作られる分割は有限なので、§6.2 の機械がそのまま動きます — 穴の証人は貼り付けられる文字列で返ってきます。",
    ...
    (
        ...
        [
            "**`allocate(配る額, 累計, 全体)` は、定数でないもので割れる唯一の場所です。** 答えは `配る額 × 累計 ÷ 全体` を下に丸めた、単位の整数。",
            "**差にすると、配った合計が総額にぴったり一致します。** 隣り合う行で同じ値が足されて引かれるので、端数は最後の行に寄ります。`proofs/` に定理があります（`runTotal_exact`）。",
            "**`constraint 累計 <= 全体` が要ります**（E117）。これが無いと配る分が配る額を超えることがあり、区間も二つの範囲の積になってしまいます。鎖はつながるので、二行に分けて書けます。",
        ],
